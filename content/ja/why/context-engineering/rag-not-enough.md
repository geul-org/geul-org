...
埋め込み類似度の構造的限界は[なぜ埋め込みベクトルでは駄目なのか](/ja/why/not-embedding-vector/)でさらに詳しく論じている。
...
機械的フィルタリングの原理は[なぜ機械的検証が必要なのか](/ja/why/mechanical-verification/)で、情報間の矛盾検出は[なぜ整合性チェックが必要なのか](/ja/why/consistency-check/)でそれぞれ論じている。
...
これらの条件が具体的にどのように実装されるかは[なぜ意味整列インデックスなのか?](/ja/why/semantically-aligned-index/)で論じている。
...
自然言語の構造的欠陥がなぜ幻覚を生むのかは[なぜ自然言語は幻覚を生むのか?](/ja/why/natural-language-hallucination/)で分析している。
