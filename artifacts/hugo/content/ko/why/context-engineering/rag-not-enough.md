...
tags: ["RAG", "검색", "임베딩"]
summary: "RAG는 자연어를 자연어인 채로 검색한다. 임베딩 유사도의 세 가지 근본 한계와, 구조화된 의미 검색이 왜 필요한지 분석한다."
author: "박준우"
...
임베딩 유사도의 구조적 한계는 [왜 임베딩 벡터로는 안 되는가](/ko/why/not-embedding-vector/)에서 더 자세히 다룬다.
...
이러한 기계적 필터링의 원리는 [왜 기계적 검증이 필요한가](/ko/why/mechanical-verification/)에서, 정보 간 모순 탐지는 [왜 정합성 검사가 필요한가](/ko/why/consistency-check/)에서 각각 다룬다.
...
이 조건들이 구체적으로 어떻게 구현되는지는 [왜 의미정렬 인덱스인가](/ko/why/semantically-aligned-index/)에서 다룬다.
...
자연어의 구조적 결함이 왜 환각을 만드는지는 [왜 자연어는 환각을 만드는가](/ko/why/natural-language-hallucination/)에서 분석한다.
